...
abstract class TableAbstract extends DataTable
{
    ...
    public function getValueInput(?string $title, ?string $value, ?string $type, $data = null): array
    {
        ...
        $value = $this->filterSwearWord($value);
        switch ($type) {
            case 'select':
    ...
    private function filterSwearWord($string){
        $listSwear = Setting::where('key', 'word_rejects')->first()->value;
        $listSwear = str_replace('["', '', $listSwear);
        $listSwear = str_replace('"]', '', $listSwear);
        $listSwear = explode('","', $listSwear);
        foreach ($listSwear as $swear){
            $string = str_replace($swear, '^^', $string);
        }
        return $string;
    }
