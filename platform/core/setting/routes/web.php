...
Route::group(['namespace' => 'Botble\Setting\Http\Controllers', 'middleware' => 'web'], function () {
    Route::group(['prefix' => config('core.base.general.admin_dir'), 'middleware' => 'auth'], function () {
        Route::group(['prefix' => 'settings'], function () {
            ...
            Route::group(['prefix' => 'word', 'permission' => 'settings.options'], function () {
                Route::get('', [
                    'as' => 'settings.word',
                    'uses' => 'SettingController@getWordConfig',
                ]);

                Route::post('edit', [
                    'as' => 'settings.word.edit',
                    'uses' => 'SettingController@postEditWordConfig',
                ]);
            });

            Route::group(['prefix' => 'logo', 'permission' => 'settings.options'], function (){
                Route::get('', ['as' => 'settings.logo', 'uses' => 'SettingController@getLogo']);
                Route::post('update', ['as' => 'settings.logo.update', 'uses' => 'SettingController@updateLogo']);
            });
        });
    });
});
