...
if (!function_exists('date_format_korean')) {
     ...
     * @author Sang Nguyen
     */
    function date_format_korean($time, $format = 'Y-m-d h:m:i A')
    {
        if (empty($time)) {
            return $time;
        }
        $timeFormat = Carbon::parse($time)->format('Y-m-d h:m:i A');
        $text = str_replace("AM","오전",$timeFormat);
        $text = str_replace("PM","오후",$text);
        $arrStr = explode(" ",$text);
        $strDateTime = $arrStr[0] . ' ' . $arrStr[2]  . ' ' . $arrStr[1];
        return $strDateTime;
    }
}
