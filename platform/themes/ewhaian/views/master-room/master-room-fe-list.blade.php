    ...
    <div class="container">
      <div class="sidebar-template">
          ...
          <div class="category-menu">
            ...
            <ul class="category-menu__links">
              <li class="category-menu__item active">
                <a href="{{route('masterRoomFE.list') }}" title="{{__('master_room.master_room_list')}}">{{__('master_room.master_room_list')}}</a>
              </li>
              <li class="category-menu__item">
                <a href="{{route('masterRoomFE.create')}}" title="{{__('master_room.create_master_room')}}">{{__('master_room.create_master_room')}}</a>
              </li>
            </ul>
          </div>
        ...
        <div class="sidebar-template__content">
          ...
          <h3 class="title-main">{{__('master_room.master_room_list')}}</h3>
          @if (session('success'))
          ...
          <div class="event-list">
            <div class="row">
              @if(count($masterRoom) > 0 )

              @foreach ($masterRoom as $item)
                ...
                <div class="item">
                  <div class="item__image">
                        <div class="item__image mb-10">
                            ...
                            @endif
                                <a href="{{route('masterRoomFE.edit',['id'=>$item->id])}}" title="{{ strip_tags($item->title) }}">
                                ...
                                <a  href="javascript:void(0)" class="btn_remove_image deleteRoom" title="{{__('master_room.remove_room')}}" data-toggle="modal" data-value="{{$item->id}}" data-target="#confirmDelete" >
                  ...
                  <div class="item__caption">
                    <h4 class="item__title">
                      <a href="{{route('masterRoomFE.edit',['id'=>$item->id])}}" title="{{ strip_tags($item->title) }}"> <div  style="  white-space: nowrap; overflow: hidden; text-overflow:ellipsis;">{{ strip_tags($item->title)}} </div> </a>
                    </h4>
              ...
              <div class="col-lg-4 col-md-6">
                  <div class="item__desc">
                    <br>
                    <h3>{{__('master_room.no_room')}}</h3>
                  </div>
              </div>
              @endif
            </div>

            {!! Theme::partial('paging',['paging'=>$masterRoom ]) !!}
        ...
        <div class="d-lg-flex align-items-start flex-grow-1">
          <div class="form-group form-group--1 flex-grow-1 mr-lg-30 mb-3">
            <label for="hint" class="form-control">
              <input type=" text" id="hint" value="{{__('master_room.confirm_delete')}}" placeholder="&nbsp;" readonly>
            </label>
          </div>
        </div>
        <form action="{{route('masterRoomFE.delete')}}" method="post">
          @csrf
