<style>
  .comments__list span.icon-label.disable, .icon-label.gray {
    color: #999999 !important;
    border-color: #999999 !important;;
  }

  .comments__list span.icon-label.disable svg {
    transform: scale(0.8);
    top: -3px;
    position: relative;
  }

  .item__time span.icon-label.pink svg {
    transform: scale(0.8);
    top: -3px;
    position: relative;
  }

  .item__time .icon-label.pink {
    margin-top: 6px;
  }

  .comments__name span.icon-label{
    min-width: 0 !important;
  }
  .comments__name .icon-label.pink{
    margin-top: 0 !important;
  }

  .item__time span.icon-label{
    min-width: 0 !important;
  }
  ...
  .single .comments .item__action button {
    vertical-align: middle;
  }
  .single .comments .item__action button span {
    vertical-align: middle;
  }
...
<div class="comments">
  ...
  @if($canViewComment ?? false)
    ...
    <p class="mt-1" style="font-size: 0.85714em;">
    ...
    <div class="comments__toggle-content" style="display: block;">
      @if (session('success'))
        <script>
          if (window.location.pathname.search('garden') > 0){
            $('body').css('overflow', 'hidden')
          }
        </script>
        <div class="alert alert-success alert2 alert-comment">
          <div class="alert-con">
              {{ session('success') }}
              <button class="btn btn-primary c-alert" onclick="$('body').css('overflow', 'auto')">확인</button>
          </div>
        </div>
      @endif
      @if (session('err'))
        <div class="alert alert-danger alert-comment" style="display: block; margin-top: 5px">
          {{ session('err') }}
        ...
        @foreach ($comments as $item)
          ...
          <div class="item">
            <div class="item__head d-flex flex-wrap align-items-center justify-content-between">
              <div class="item__group-ip-time-delete">
                <div class="item__time flex-c">
                  <div style="vertical-align: middle">
                  @if(Route::currentRouteName() != 'contents.details')
                  ...
                  @if ($item->anonymous == 1 && !is_null($item->ip_address))
                    <p><a target="_parent" onclick="ipPopClick('{{preg_replace('/([0-9]+\.[0-9]+\.[0-9]+)\.[0-9]+/', '\1.xxx', $item->ip_address)}}')" >
                      ...
                      <span class="icon-label span_show_id"
                            style="cursor: pointer;font-size: 10px;line-height: 2em;width: 5em;height: 2em;border-radius: 5px;background-color: #f4f4f4;border: 1px solid #f4f4f4;">
                        IP
                      </span>
                      </a>
                    </p>
                  @endif
                  <p>{{getName($item->members->nickname ??  __('comments.admin'), $item->members->id_login ??  __('comments.admin'), $canViewCommenter ?? false) }}</p>

                  <p style="margin-left: 0.71429em">{{$item->created_at}}</p>
                </div>
                @if( getStatusDateByDate($item->created_at) == "Today" ) <span class="icon-label"
                                                                               style="margin-left: 10px; margin-top: 6px;">N</span> @endif
              ...
              @if($item->is_deleted == 0)
                @if(auth()->guard('member')->check())
                  <div class="item__action">
                    @if(empty($object) or $object->active_empathy > 0)
                      <div class="single__date">
                        <a href="javascript:void(0)" class="modal-comment-like comment-{{$item->id}}-like mr-sm-1"
                          ...
                          <span id="like_count{{$item->id}}" style="vertical-align: middle;">{{ $item->likes_count ?? 0 }}</span>
                        </a>
             ...
             {{-- sheet garden - 74 --}}
              @if($item->is_deleted == 0)
                @if( $item->anonymous == 0 ||
                      ...
                      7 == auth()->guard('member')->user()->role_member_id
                      //  || Route::currentRouteName() === 'gardenFE.details'
                    )
                    {!! $createBy == $item->member_id ? '* ' : null !!}
                    {!!checkCommentFromSympathy($item->id)!!}{!! html_entity_decode($item->content) !!}
                    @if($item->file_upload)
                      <br>
                    @php
                        $listFile = explode(',', $item->file_upload)
                    @endphp
                      ...
                      <div style="display: flex">
                        <div class="item__time flex-c">
                          ...
                          @if( $subitem->anonymous == 1 && !is_null($subitem->ip_address) )
                            <p style="margin-top: 0;"><a target="_parent" onclick="ipPopClick('{{preg_replace('/([0-9]+.[0-9]+.[0-9]+).[0-9]+/', '1.xxx', $item->ip_address)}}')" >
                              @php
                                $randomIp = mt_rand(0, 255) . "." . mt_rand(0, 255) . "." . mt_rand(0, 255) . "." . mt_rand(0, 255);
                              @endphp
                          ...
                          @endif
                          <p style="margin-top: 0;">{{$subitem->created_at}}</p>
                        </div>
                        @if( getStatusDateByDate($subitem->created_at) == "Today" ) <span class="icon-label"
                                                                                          style="margin-left: 10px !important; margin-top: 6px;">N</span> @endif

                        @if($subitem->is_deleted == 0)
                          @if(auth()->guard('member')->check())
                      ...
                      <div class="item__action">
                        <div class="single__date">
                          <a href="javascript:void(0)" class="modal-comment-like comment-{{$subitem->id}}-like mr-sm-1"
                             @if( getLikeCommentToMember($subitem->id,auth()->guard('member')->user()->id)) data-like="true-sub" @else data-like="false-sub" @endif data-comment_id="{{$subitem->id}}">
                            <svg
                              class="icon like" width="20" height="16.56" aria-hidden="true"
                          ...
                          <a href="javascript:void(0)" class="modal-comment-dislike comment-{{$subitem->id}}-dislike mr-sm-1"
                             @if( getDislikeCommentToMember($subitem->id,auth()->guard('member')->user()->id)) data-dislike="true-sub"
                             @else data-dislike="false-sub"
                             @endif data-comment_id="{{$subitem->id}}"
                          >
                            <svg
                              class="icon" width="20" height="16.56" aria-hidden="true"
                    ...
                    <div class="item__content">
                      ...
                      @if($subitem->is_deleted == 0)
                        @if( $subitem->anonymous == 0 ||
                            ...
                            {!! $createBy == $subitem->member_id ? '* ' : null !!}
                            {!!checkCommentFromSympathy($subitem->id)!!}{!! html_entity_decode($subitem->content) !!}
                        @else
                            <div style="display: flex;color: #EC1469;">
                              {!! $createBy == $subitem->member_id ? '* ' : null !!}
                              {{__('comments.this_is_a_secret_comment')}}
              ...
              @if(auth()->guard('member')->check())
                <form action="{{route($route)}}" method="POST">
                  ...
                  <div class="item__form">
                    ...
                    <div class="row">
                      <div class="col-6">
                        @if (Route::currentRouteName() === 'gardenFE.details' || Route::currentRouteName() == 'contents.details')
                          <input type="hidden" class="form-check-input" id="secret{{$item->id}}" value="1"
                                 name="is_secret_comments"/>
                        @endif
                        ...
                        @if (
                            ...
                            Route::currentRouteName() != 'contents.details'
                        )
          ...
          <div class="comments__form"><br>
            <div class="d-flex">
              ...
              <div style="margin-left: 20px ">
                {!! Theme::partial('upload_link_comment',['link'=> '',
                'file_upload' => '' ]) !!}
              </div>
              @endif
            </div>
...
@if ($canViewComment ?? false)
  @foreach ($comments as $item)
    <!-- Modal -->
    <div class="modal fade modal--confirm" id="reportPost2{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered  modal-lg" role="document">
        ...
        <div class="modal-body">
          <form action="{{route('ewhaian.report')}}" method="POST" id="report_form{{$item->id}}">
            ...
            <div>
              ...
              <input type="hidden" name="link" value="{{Request::path()}}"> <!-- link comment -->
              <input type="hidden" name="id_post" value="{{$item->id}}"> <!-- type post -->
              <input type="hidden" name="reported_id" value="{!! $item->member_id ?? null !!}"> <!-- reported_id -->
              <div class="d-lg-flex align-items-center mx-3">
                <div class="d-lg-flex align-items-start flex-grow-1">
                  <div class="form-group form-group--1 flex-grow-1 mb-3">
                    <div class="md-form">
    ...
    @foreach ($item->getAllCommentByParentsID($item->id) as $subitem)
      <!-- Modal -->
      <div class="modal fade modal--confirm" id="reportPost2{{$subitem->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered  modal-lg" role="document">
          <div class="modal-content" style="padding-right: 0px">
            <div class="modal-body">
              <form action="{{route('ewhaian.report')}}" method="POST" id="report_form{{$subitem->id}}">
                ...
                <div>
                  ...
                  <input type="hidden" name="reported_id" value="{{$subitem->member_id}}"> <!-- reported_id-->
                  <div class="d-lg-flex align-items-center mx-3">
                    <div class="d-lg-flex align-items-start flex-grow-1">
                      <div class="form-group form-group--1 flex-grow-1 mb-3">
                        <div class="md-form">
...
{!! Theme::partial('popup.like_dislike_comment', [
    ...
    'route_sympathy_permission_on_comment' => $route_sympathy_permission_on_comment ?? '/',
    'idDetail' => $idDetail
]) !!}
@if(Route::currentRouteName() === 'gardenFE.details')
  <!-- Modal -->
...
<script>
  ...
  $(document.body).on('click', '.submitByPwdPostPopup', function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.check == true) {
          @if(Route::currentRouteName() == 'egardenFE.details')
  ...
  $('.c-alert').click(function(){
      $('.alert-comment').css('display','none');
  });
