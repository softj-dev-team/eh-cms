...
<div class="comments">
  ...
  <div style="display: flex; position: relative;margin-bottom: 40px;z-index:1000">
    ...
    @switch(Route::currentRouteName() )
      @case('masterRoomFE.details')
        @if(!$isMasterRoomReply)
        <div class="account__icon" style="position: absolute; right: 0">
          <a href="{{ route('masterRoomFE.reply.create', ['id' => $idDetail, 'idCategory' => $idCategory]) }}" title="답글" data-toggle="modal" data-value="{{$idDetail}}">
            <span class="icon-label" style="border-radius: 5px;">답글</span>
          </a>
        </div>
        @endif
        @if($canEdit ?? false)
      ...
      @default
        @if(!$isMasterRoomReply)
        <div class="account__icon" style="position: absolute; right: 130px">
          <a href="{{ route('masterRoomFE.reply.create', ['id' => $idDetail, 'idCategory' => $idCategory]) }}" title="답글">
            <span class="icon-label">답글</span>
          </a>
        </div>
        @endif
        @if($canEdit ?? false)
          <div class="account__icon" style="position: absolute; @if($canDelete ?? false) right: 65px @else right: 0px @endif">
            @if(!$isMasterRoomReply)
              <a href="{{route($editItem, ['id'=> $idDetail])}}" title="수정" target="_parent"
                 @if(isset($show_pwd_post) && $show_pwd_post == 1) data-toggle="modal" data-target="#confirmPwdPost" @endif
              >
            ...
            @else
              <a href="{{route('masterRoomFE.reply.edit', ['id'=> $idDetail])}}" title="수정" target="_parent">
              <span class="icon-label">
              <!-- 수정 -->
                {{__('comments.edit')}}
              </span>
  ...
  @if($canViewComment ?? false)
    ...
    <div class="comments__toggle-content" style="display: block;">
      ...
      <div class="comments__list">
        @if (!is_null($top_comments) && count($top_comments) > 0)
            ...
            <div class="item__head d-flex flex-wrap align-items-center justify-content-between">
              ...
              @if(auth()->guard('member')->check())
                <div class="item__action">
                  ...
                  <button data-target="#reportPost2{{$item->id}}" data-toggle="modal">
...
{{--              @if( $item->anonymous < 1 || Route::currentRouteName() === 'masterRoomFE.detail' || Route::currentRouteName() === 'masterRoomFE.reply.detail')--}}
{{--                {!! $item->content !!}--}}
{{--              @else--}}
{{--                <div>--}}
              ...
              <div class="comments__list" style="line-height: 1.71429em;letter-spacing: 0.5px;border-radius: 4px;">
                @foreach ($item->getAllCommentByParentsID($item->id) as $subitem)
                  ...
                  <div class="item"
                    ...
                    <div class="item__head d-flex flex-wrap align-items-center justify-content-between">
                      ...
                      <div class="item__action">
                        <div class="single__date">
                          <a href="javascript:void(0)" class="modal-comment-like comment-{{$subitem->id}}-like"
                            ...
                            <svg
                              class="mr-10 icon like" width="20" height="16.56" aria-hidden="true"
                        ...
                        <div class="single__date">
                          <a href="javascript:void(0)" class="modal-comment-dislike comment-{{$subitem->id}}-dislike"
                            ...
                            <svg
                              class="mr-10 icon" width="20" height="16.56" aria-hidden="true"
                        ...
                        <button data-target="#reportPost2{{$subitem->id}}" data-toggle="modal">
                          <img src="{{Theme::asset()->url('img/dependence.svg')}}"
                               alt="{{__('comments.declaration')}}" width="18.008" height="13.506">
                          {{__('comments.declaration')}}
                        </button>
                    ...
                    <div class="item__content">
{{--                      @if( $subitem->anonymous < 1 || Route::currentRouteName() === 'contents.details' || Route::currentRouteName() === 'masterRoomFE.reply.detail')--}}
{{--                        {!! $subitem->content !!}--}}
{{--                      @else--}}
{{--                        <div>--}}
{{--                          <div style="display: flex;color: #EC1469;">--}}
...
{{--                                            <div>{{__('comments.report_info')}}--}}
              ...
              @if(auth()->guard('member')->check())
                ...
                <form action="{{route($route)}}" method="POST">
                  <input type="hidden" name="isMasterRoomReply" value="{{ $isMasterRoomReply }}">
                  @csrf
...
{{--              </textarea >--}}
            ...
            <div class="row">
              <div class="col-6">
                @if (Route::currentRouteName() === 'masterRoomFE.details')
