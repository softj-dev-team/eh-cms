...
class ApiController extends BaseController
{
    ...
    public function updateProfile(Request $request, BaseHttpResponse $response) {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'avatar' => 'nullable|mimes:jpeg,png,gif,bmp,svg,webp,jpg',
        ]);
    ...
    public function tokenDevice(Request $request, BaseHttpResponse $response) {
        $data = $request->only(['device']);

        $request->currentMemberToken->device = $data['device'];
        $request->currentMemberToken->save();

        return $response->setMessage(trans('plugins/member::member.update_token_successful'));
    }
}
