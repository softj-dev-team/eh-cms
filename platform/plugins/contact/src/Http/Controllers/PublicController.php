...
class PublicController extends Controller
{
    ...
    {
        Theme::setTitle(__('eh-introduction').' | '.__('eh-introduction.contact'));
        Theme::breadcrumb()->add(__('eh-introduction'), route('eh_introduction.list'))->add(__('eh-introduction.contact'), 'http:...');
        $categories = CategoriesIntroduction::where('status', 'publish')->orderby('created_at', 'DESC')->get();

        return Theme::scope('contact-us.index',['categories'=>$categories])->render();
