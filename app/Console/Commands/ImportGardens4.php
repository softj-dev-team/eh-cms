...
class ImportGardens4 extends Command
{
    use ConnectSql;
    use ConnectMySql;
     ...
     * @var string
     */
    protected $signature = 'import:gardens-4';
     ...
     * @var string
     */
    protected $description = 'Import garden_category_id 4 from old DB';
    ...
    protected function insertEvents($total, &$index) {
        ...
        while ($count < $total) {
            ...
            $tsql = "SELECT [EWHA_JOB_POST].[BP_IDX],[MEM_IDX],[BP_TITLE],[BP_CONTENT],[BP_COUNT],[BP_REPLE]
                  ,[BP_DATE],[BP_TYPE],[BP_MOBILE],[MEM_IP],[TEMP_IDX],[BP_GOOD],[BP_GUBAK]
                  ,[BP_LOVE],[BP_IMG],[S_PWD]
              FROM [BIWON].[dbo].[EWHA_JOB_POST]
              LEFT JOIN [BIWON].[dbo].[EWHA_BIWON_SECRET] ON EWHA_BIWON_SECRET.BP_IDX = EWHA_JOB_POST.BP_IDX
              ORDER BY [EWHA_JOB_POST].[BP_IDX]
              OFFSET $offset ROWS
            ...
            $sql = 'INSERT IGNORE INTO `gardens_bk_4` (`id`, `title`, `detail`, `notice`, `lookup`, `right_click`,
    ...
    protected function insertComment($ids) {
        ...
        foreach ($ids as $id) {
            ...
            $sql = 'INSERT IGNORE INTO `comments_gardens_bk_4` (`id`, `gardens_id`, `parents_id`, `content`,
    ...
    protected function insertReactStatusComment($ids, $reactStatus) {
        ...
        switch ($reactStatus) {
            case 'like':
                $table = 'JOB_REPLE_GOOD_LOG';
                $isDislike = 0;
                break;

            case 'dislike':
                $table = 'JOB_REPLE_BAD_LOG';
                $isDislike = 1;
                break;
        }
        ...
        foreach ($ids as $id) {
            ...
            $sql = 'INSERT IGNORE INTO `sympathy_gardens_bk_4` (`gardens_id`, `member_id`,
